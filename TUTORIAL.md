# msgspec-serde Tutorial

`msgspec-serde` generates Python modules from FlatBuffers IDL (`.fbs`)
...
Start with a model and view for one table. The later examples add owned models,
subclass validation, JSON and MessagePack codecs, unions, nested FlatBuffers,
and dynamic payloads. The finished example is in
...
The fourth line is the `Monster` model encoded as JSON.
...
```shell
python -m pip install --user msgspec-serde
```

With `--user`, pip usually places the `msgspec_flatc` executable in
`~/.local/bin`. If your shell cannot find it, add that directory to `PATH`.
...
This command creates `generated/tutorial/monster.py`. The `Tutorial` namespace
becomes the `tutorial/` package directory. The `Monster` root table becomes the
`monster.py` module.

By default, each IDL definition gets its own module under the namespace package,
as with `flatc --python`. For example, `Tutorial.Weapon` becomes
`generated/tutorial/weapon.py`. The root module re-exports unambiguous
dependency names, so the tutorial can import `Weapon` from `tutorial.monster`.
The command prints the root table's module path.
...
directory. One-file generation rejects two definitions with the same short
name. The default layout keeps those definitions in separate modules.

One command may accept several schema paths. Use `-I` to add directories for
FlatBuffers `include` statements. An included schema and every schema that
imports it must use the same one-file setting.
...
`msgspec_serde.flatbuffer` encodes generated models and decodes roots as models
or views. Generated types do not define format methods. Create `demo.py`:
...
With the default `initial_size=0`, the encoder estimates a bounded initial
allocation. Pass a positive value to start with that many bytes and skip the
estimate:
...
The encoder grows the buffer if the initial allocation is too small.

## 5. Read fields without materializing the object

A view borrows the FlatBuffer and decodes each field on access:
...
Views support concurrent reads while the backing buffer remains unchanged. Two
threads that first access the same field may both construct its cached value.
Later accesses reuse the published value. Iterating a vector to completion
publishes the vector's full cache.

Views accept contiguous buffer objects, including `bytes`, `bytearray`,
`memoryview`, and memory-mapped files. A view exposes the buffer through a
read-only `memoryview`, but another reference can still modify a mutable
backing object such as a `bytearray`. Do not modify the backing object while a
view uses it.
...
Both forms copy the entire object graph. Byte vectors become `bytes`. Numeric
vectors become writable NumPy arrays that own their storage. Other vectors
normally become Python lists; keyed table vectors become dictionaries as
described below. Each call returns an independent model.
...
When a table marks one field as `(key)`, a vector of that table becomes a
dictionary in the model and a read-only `TableMap` in the view. Each dictionary
key must equal the corresponding model's key field:
...
Encoding sorts keyed vectors as required by FlatBuffers. `TableMap` uses binary
search without materializing the entire mapping.

## 7. Add application validation

Subclass a generated model to add validation, methods, or Python-only state.
Redeclare an inherited field with the subclass type to decode that nested field
as a subclass:
...
`ClassVar` values and attributes in an instance's `__dict__` are Python-only.
The `dict=True` class option enables the instance dictionary. Neither msgspec
nor FlatBuffers serializes these values. Every other annotation creates a
msgspec field. FlatBuffer conversion rejects a subclass whose msgspec fields do
not match the generated model.

## 8. Encode JSON and MessagePack

Use the package codecs to encode and decode generated models or any
`msgspec.Struct` type that contains NumPy arrays:
...
The codecs also round-trip subclasses. Pass the subclass as `type` to run its
`__post_init__()` validation during decoding:
...
For a Struct that was not generated from IDL, the first encode or decode
compiles a native plan from the field annotations and caches it. Call
`register()` to compile the plan eagerly:
...
Each returned tuple lists the fields that msgspec must handle. Fallback is per
field: an unsupported annotation, such as a dictionary, `bytes`, or a custom
type, sends that field through typed msgspec conversion. Supported fields in
the same object graph remain native. A Struct with `array_like=True`,
`forbid_unknown_fields=True`, or a non-string tag uses msgspec for the entire
document.
...
Supported fields remain native when standard msgspec options are enabled.
`order="sorted"` sorts Struct field names. `order="deterministic"` keeps Struct
declaration order and sorts unordered values in fallback fields. Native
processing also covers scalar UUID and Decimal fields, generated enums, and
`strict=False` coercion for scalars and numeric vectors. `enc_hook`, `dec_hook`,
JSON `float_hook`, and MessagePack `ext_hook` run only for fallback fields.
...
Materialized fields use Python union types:
...
The encoder derives each FlatBuffers discriminator from the concrete Python
type. A view exposes the selected value and its discriminator:
...
A union fixes every permitted type in the IDL. A dynamic field instead stores a
type name beside a byte vector. Generated extension modules register types at
runtime, so a new payload type does not require an envelope-schema change.

Save an extension type as `schemas/metric.fbs`:
...
Importing the generated extension module registers `Tutorial.Metric`. The
envelope module exports `DynamicEnvelopePayload`, which stores the dynamic
value and its type name:
...
`Metric` remains an untagged msgspec model. The package JSON codec encodes and
decodes the wrapper:
...
```python
from msgspec_serde import DynamicModelOverrides
...
`DynamicModelOverrides` maps generated models to application subclasses. The
`|` operator returns a new mapping; `|=` updates the mapping on its left. The
right-hand value wins when both mappings contain the same generated model.
Without an override, decoding constructs the generated model.

An unknown type inside the allowed `Tutorial.*` namespace remains opaque. Model,
JSON, and FlatBuffer round trips preserve its type name and bytes:
...
Tags outside `Tutorial.*` are rejected. The runtime never interprets a wire tag
as a Python import path. To load a registered type lazily, map its trusted tag
to a module:

```python
from msgspec_serde import register_dynamic_module
...
Use an unannotated `[ubyte]` vector when the schema does not declare a payload
...
`Extension.data` is `bytes` in the model. The generated `data_as()` helper opens
the vector as a chosen view type without copying it:
...
The application defines what `type_id` means. Repeated `data_as()` calls with
the same view type and `size_prefixed` setting return the cached view.
...
A size-prefixed reader restricts the view to the payload length declared by the
prefix. The view excludes trailing bytes and later concatenated frames.
...
Generated modules support tables, structs, fixed arrays, enums, optional
scalars, strings, unions, and typed or dynamic nested FlatBuffers. Vectors may
contain scalars, bytes, strings, tables, structs, or unions. One `.fbs` file may
declare types in several namespaces, and schemas may include other schemas.
...
The generator rejects an unsupported schema instead of producing a partial
Python API.
