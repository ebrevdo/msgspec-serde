...
INSTALLED_VERSION = msgspec_flatbuffers.__version__


@pytest.fixture(autouse=True)
def reset_version_check(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(msgspec_flatbuffers, "__version__", "1.2.3")
    monkeypatch.setattr(msgspec_flatbuffers, "warn_on_older_runtime", True)
    monkeypatch.setattr(msgspec_flatbuffers, "_warned_version_pairs", set())
...
def test_same_or_older_generated_version_does_not_warn(
    ...
    with warnings.catch_warnings():
        ...
        msgspec_flatbuffers._check_generated_code_version(generated_version)


def test_older_minor_runtime_warns_once() -> None:
    with pytest.warns(GeneratedCodeVersionWarning, match="runtime 1.2.3 is older"):
        msgspec_flatbuffers._check_generated_code_version("1.3.0")
...
def test_older_minor_runtime_warning_can_be_disabled(
    ...
    monkeypatch.setattr(msgspec_flatbuffers, "warn_on_older_runtime", False)
