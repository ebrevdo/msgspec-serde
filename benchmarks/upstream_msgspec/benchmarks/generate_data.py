...
class Generator:
    ...
    def __init__(self, capacity, seed=42, vector_length=DEFAULT_VECTOR_LENGTH):
        self.capacity = capacity
        self.random = random.Random(seed)
        self.vector_random = random.Random(seed + 1)
    ...
    def _node_base(self, is_dir):
        ...
        d = {
            ...
            "int_values": [
                self.vector_random.randint(-(2**31), 2**31 - 1)
                for _ in range(self.vector_length)
            ],
            "float_values": [
                self.vector_random.uniform(-1_000_000.0, 1_000_000.0)
                for _ in range(self.vector_length)
            ],
        }
...
def make_filesystem_data(n, vector_length=DEFAULT_VECTOR_LENGTH):
    return Generator(n, vector_length=vector_length).generate()
